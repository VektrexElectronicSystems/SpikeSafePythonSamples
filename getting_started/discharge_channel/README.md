...
### Overview
Operates SpikeSafe as a DC current source with single output current, stops current output, and then uses the [spikesafe-python](https://pypi.org/project/spikesafe-python/) [`spikesafe_python.get_spikesafe_channel_discharge_time()`](../../spikesafe_python_lib_docs/Discharge/spikesafe_python.get_spikesafe_channel_discharge_time/) and [`spikesafe_python.wait()`](../../spikesafe_python_lib_docs/Threading/wait/) functions to properly discharge the load with the intent of running a second test. Afterwards, the same process is repeated for the second and final test, then the SpikeSafe Discharge Complete query is used again with the intent of ensuring that it is safe to be disconnected.
